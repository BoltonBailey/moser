...
\begin{lemma}[Soundness of the containment test]
  \label{lem:contains-sound}
  \lean{ConvexPolygon.mem_realHull_of_contains,
    ConvexPolygon.realHull_subset_realHull}
  \leanok
  \uses{def:contains, def:convex-polygon}
  Any rational point accepted by the containment test of a convex polygon
  $Q$ lies in the real convex hull of $Q$'s vertices. Consequently, if the
  subset test accepts $(P, Q)$, then the real region of $P$ is contained in
  that of $Q$.
\end{lemma}
\begin{proof}
  \leanok
  Fan triangulation from vertex $0$: by induction on the number of
  vertices, a point weakly to the left of every directed edge lies either
  in the hull of the first $n-1$ vertices (if it is weakly left of the
  closing diagonal) or in the last fan triangle, whose membership is
  witnessed by explicit barycentric coordinates (subtriangle areas over the
  total area). The fan triangles are positively oriented by the strict
  convexity invariant of the polygon.
\end{proof}
...
\begin{proof}
  \leanok
  The hull decomposes into the fan triangles from vertex $0$: the fan
  covers the hull by the same induction as in
  Lemma~\ref{lem:contains-sound}, and the triangles overlap only in null
  diagonal segments. The separating diagonals are supplied by
  \emph{diagonal positivity} --- every vertex lies strictly left of every
  diagonal from vertex $0$ --- which follows from the edge conditions by
  transitivity of the angular order inside an open half-plane, an
  instance of the Grassmann--Pl\"ucker relation. Each fan triangle is the
  affine image of the standard simplex, whose volume $1/2$ is computed by
  reflecting it onto the other half of the unit square; the linear part
  scales by its determinant, the cross product of the edge vectors.
  Summing and telescoping the fan cross products yields the cyclic
  shoelace sum, which is nonnegative for a counterclockwise polygon.
\end{proof}
...
\begin{definition}[Superset removal]
  ...
  Process the polygons of the list sequentially, maintaining a list of
  retained polygons: a polygon $p$ is dropped when some already-retained
  polygon is a subset of $p$, and when $p$ is retained, any retained
  polygons having $p$ as a subset are dropped. A retained subset $q$
  already witnesses Invariant~2 whenever $p$ does. (The sequential form
  matters: the vertex-containment test is not antisymmetric --- two
  distinct polygons, e.g.\ with cyclically rotated vertex lists, can each
  contain the other --- and the symmetric filter ``drop $p$ whenever some
  other $q \subseteq p$ is present'' would delete both members of such a
  pair, potentially removing the last witness of Invariant~3.)
\end{definition}

\begin{proposition}[Superset removal preserves the working set invariant]
  \label{prop:supersetRemoval-correct}
  \lean{Moser.WorkingSet.ContainsInitialWorm.supersetRemoval}
  \leanok
...
\begin{remark}[State of the Lean formalisation]
  \label{rem:lowerBoundLeanState}
  \uses{thm:lower-bound, prop:wormAdding-correct}
  The Lean development does not assert Theorem~\ref{thm:lower-bound}; it proves
  the reduction \texttt{Moser.areaThreshold\_le\_moserCoverNumber\_of\_run},
  which derives the bound from a certificate
  $\exists s,\ s.\mathtt{Sound} \wedge s.\mathtt{polygons} = []$. Two things are
  outstanding: that certificate (Proposition~\ref{prop:terminating-sequence}),
  and Proposition~\ref{prop:wormAdding-correct}, which is carried through the
  Lean statements as an explicit hypothesis
  \texttt{Moser.WorkingSet.WormAddingSound} rather than assumed silently. Every
  other statement in the development is proved; the file has no \texttt{sorry}.
\end{remark}
...
\begin{lemma}[The node bound is automatic]
    \label{lem:gridNodesInDisc}
    \lean{Moser.CompactnessOutline.gridPolygonalWorms_eq_bounded_nodes}
    \uses{def:gridPolygonalWorm, def:pinnedWorm}
    \leanok
    Requiring in addition that the nodes $p_0, \dots, p_k$ lie within distance
    $1$ of the origin does not change the set of $(k,\delta)$-grid polygonal
    worms. Indeed the interpolant is $1$-Lipschitz and pinned at the origin, so
    its whole range lies in the closed unit disc, and each node $p_n$ with
    $n \le k$ is a value of it: $p_n = \tilde f(n/k)$.
\end{lemma}
...
\begin{definition}[$\varepsilon$-net of worms]
    ...
    \lean{Moser.CompactnessOutline.IsWormEpsilonNet}
...
\begin{theorem}[A finite grid $\varepsilon$-net exists]
    ...
    \lean{Moser.CompactnessOutline.finiteWormEpsilonNet}
    \leanfile{Moser/Real/CompactnessOutline.lean}
    \uses{def:gridPolygonalWorm, def:epsilonNet}
    \leanok
    For every $\varepsilon > 0$ there exist $k \in \NN$ and $\delta > 0$ such
    that the set $S_\varepsilon$ of all $(k,\delta)$-grid polygonal worms is
    finite and is an $\varepsilon$-net of worms.
\end{theorem}
\begin{proof}
    \uses{def:pinnedWorm, def:gridPolygonalWorm, def:thickening,
      def:epsilonNet, lem:gridNodesInDisc}
    ...
    these $k$ and $\delta$, the set $S_\varepsilon$ is finite: by
    Lemma~\ref{lem:gridNodesInDisc} the nodes lie in the closed unit disc, so each of
    the finitely many nodes ranges over the finitely many grid points of
    $\delta\ZZ \times \delta\ZZ$ there, and the nodes determine the worm.
    ...
    $(k,\delta)$-grid polygonal worm, hence $\tilde f \in S_\varepsilon$. Comparing $f$ to $\tilde f$
...
\begin{theorem}[Lower bound on the Moser number]
    ...
    \lean{Moser.CompactnessOutline.moserCoverNumber_lowerBound}
...
\begin{theorem}[Upper bound on the Moser number]
    ...
    \lean{Moser.CompactnessOutline.moserCoverNumber_upperBound}
...
\begin{remark}[Steiner gap, and what replaces it]
    \label{rem:minkowskiSteinerGap}
    \uses{def:minimalCover, def:thickening}
    The natural way to control the gap between the bounds of
    Theorems~\ref{thm:lowerBound} and~\ref{thm:upperBound} is the planar Steiner
    formula: for a convex body $K$ with perimeter $L$ and every
    $\varepsilon \ge 0$,
    \[
        \area\big(K^{\varepsilon}\big)
            = \area(K) + \varepsilon\, L + \pi \varepsilon^{2}.
    \]
    Mathlib has no Steiner formula (nor mixed volumes or intrinsic volumes), so
    the formalisation replaces it by the elementary dilation bound
    Theorem~\ref{thm:dilationBound}, which is weaker but suffices: it also makes
    the gap tend to $0$ with $\varepsilon$, and it needs no perimeter bound.
\end{remark}

\begin{theorem}[Dilation bound]
    \label{thm:dilationBound}
    \lean{Moser.CompactnessOutline.volume_cthickening_le_of_closedBall_subset}
    \leanok
    \uses{def:thickening}
    Let $K \subseteq \RR^2$ be convex and suppose $K$ contains a disc of radius
    $r > 0$. Then for every $\varepsilon \ge 0$,
    \[
        \area\big(K^{\varepsilon}\big)
            \le \Big(1 + \tfrac{\varepsilon}{r}\Big)^{2} \area(K).
    \]
\end{theorem}
\begin{proof}
    \leanok
    Passing to the closure changes neither side (the frontier of a convex set is
    null). A point at distance at most $\varepsilon$ from $K$ is of the form
    $x + b$ with $x \in K$ and $\lVert b\rVert \le \varepsilon$; writing
    $u = c + (r/\lVert b\rVert)\, b$ for the point of the disc in the direction
    of $b$, the point $x + b$ is the image under the homothety of centre $c$ and
    ratio $1 + \varepsilon/r$ of a convex combination of $x$ and $u$, hence of a
    point of $K$. The homothety multiplies area by $(1 + \varepsilon/r)^2$.
\end{proof}

\begin{theorem}[A priori bound $M \le \pi/4$]
    \label{thm:apriori}
    \lean{Moser.CompactnessOutline.moserCoverNumber_le_pi_div_four}
    \leanok
    \uses{def:moserNumber}
    $M \le \pi/4$.
\end{theorem}
\begin{proof}
    \leanok
    A worm $f$ is $1$-Lipschitz on $[0,1]$, so every point of its image lies
    within $1/2$ of $f(1/2)$. Translating $f(1/2)$ to the origin places every
    worm inside the disc of radius $1/2$, which is convex; so that disc is a
    cover, of area $\pi/4$.
\end{proof}

\begin{theorem}[Every cover contains a disc of radius $1/20$]
    \label{thm:coverDisc}
    \lean{Moser.CompactnessOutline.exists_closedBall_subset_of_isPlacementCover}
    \leanok
    \uses{def:minimalCover}
    Let $S$ be a family of worms containing the ``V worm'' $V$ --- the path from
    $(0,0)$ to $(1/2,0)$ to $(1/2,1/2)$ --- and let $K$ be a cover of $S$. Then
    $K$ contains a disc of radius $1/20$.
\end{theorem}
\begin{proof}
    \leanok
    $K$ contains a placed copy of $V$, hence of the convex hull of $V$, the
    triangle with vertices $(0,0)$, $(1/2,0)$, $(1/2,1/2)$. That triangle
    contains the disc of radius $1/20$ centred at $(3/10, 1/10)$: the
    barycentric coordinates $1 - 2p_0$, $2p_0 - 2p_1$, $2p_1$ of a point $p$ of
    that disc are all nonnegative. Placements are isometries, so they carry
    discs to discs.
\end{proof}
...
\begin{theorem}[Approximating the Moser number]
    ...
    \leanok
    \uses{def:moserNumber, thm:finiteEpsilonNet, thm:lowerBound, thm:upperBound,
      thm:dilationBound, thm:apriori, thm:coverDisc}
    For any target accuracy $x > 0$ there are a finite $\varepsilon$-net
    $S_\varepsilon$ of worms and a convex cover $K$ of it such that
    \[
        \area(K(S_\varepsilon)) \le M \le \area(K^{\varepsilon}),
        \qquad
        \area(K^{\varepsilon}) - \area(K(S_\varepsilon)) \le x .
    \]
\end{theorem}
\begin{proof}
    \leanok
    \uses{thm:lowerBound, thm:upperBound, thm:dilationBound, thm:apriori,
      thm:coverDisc}
    Take the grid $\varepsilon$-net of Theorem~\ref{thm:finiteEpsilonNet},
    enlarged by the V worm, and let $K$ be a convex cover of it whose area
    exceeds $\area(K(S_\varepsilon))$ by at most $\eta$; such a $K$ exists
    because the convex hull of the placed copies realises the infimum up to
    $\eta$ (no Blaschke selection theorem is needed). By
    Theorem~\ref{thm:coverDisc}, $K$ contains a disc of radius $1/20$, so
    Theorem~\ref{thm:dilationBound} gives
    $\area(K^{\varepsilon}) \le (1 + 20\varepsilon)^2 \area(K)$. Since
    $\area(K) \le \area(K(S_\varepsilon)) + \eta \le \pi/4 + \eta$ by
    Theorem~\ref{thm:apriori}, the gap is at most
    $\big((1+20\varepsilon)^2 - 1\big)\pi/4 + (1+20\varepsilon)^2 \eta$, which is
    at most $x$ for $\varepsilon$ and $\eta$ chosen explicitly in terms of $x$.
\end{proof}

\begin{corollary}[The bounds converge]
    \label{cor:convergentBounds}
    \lean{Moser.CompactnessOutline.tendsto_bounds_gap_zero}
    \leanok
    \uses{thm:approxAlgorithm}
    There are sequences $L_n \le M \le U_n$, each an area of an explicit convex
    cover of an explicit finite net of polygonal worms, with
    $U_n - L_n \to 0$.
\end{corollary}
\begin{proof}
    \leanok
    \uses{thm:approxAlgorithm}
    Apply Theorem~\ref{thm:approxAlgorithm} with $x = 1/(n+1)$.
\end{proof}

\begin{theorem}[Polygonal approximation with $m$ segments]
    \label{thm:polygonalApprox}
    \lean{Moser.CompactnessOutline.exists_polygonalWorm_approx,
      Moser.CompactnessOutline.exists_polygonalWorm_convexHull_approx}
    \leanok
    \uses{def:worm, def:thickening}
    For every worm $w$ and every $m \ge 1$ there is a worm $w'$ made of $m$
    segments with $w \subseteq (w')^{1/2m}$ and $w' \subseteq w^{1/2m}$; the same
    bounds hold for the convex hulls.
\end{theorem}
\begin{proof}
    \leanok
    Sample $w$ at the parameters $j/m$ and join consecutive samples. Consecutive
    samples are at distance at most $1/m$, so the interpolant is $1$-Lipschitz;
    every parameter is within $1/2m$ of a sample, and every point of a segment
    is within $1/2m$ of one of its endpoints.
\end{proof}

\begin{theorem}[Pruning bound]
    \label{thm:pruning}
    \lean{Moser.CompactnessOutline.worm_subset_cthickening_of_image_subset,
      Moser.CompactnessOutline.worm_subset_cthickening_minCoverLength}
    \leanok
    \uses{def:worm, def:thickening}
    Let $f$ be a worm and $0 \le a \le b \le 1$. If $f([a,b]) \subseteq A$ then
    the image of $f$ lies in the $\big(1 - (b-a)\big)$-thickening of $A$. In
    particular, if a set $P$ of points of the worm is traced on $[a,b]$ and $L$
    is the minimal length of a $1$-Lipschitz curve covering $P$, then no point of
    the worm is further than $1 - L$ from $f([a,b])$.
\end{theorem}
\begin{proof}
    \leanok
    A parameter outside $[a,b]$ is within $\max(a, 1-b) \le 1 - (b-a)$ of an
    endpoint of $[a,b]$, and $f$ is $1$-Lipschitz. The restriction of $f$ to
    $[a,b]$, reparametrised, is a $1$-Lipschitz curve of length $b - a$ covering
    $P$, so $L \le b - a$.
\end{proof}
