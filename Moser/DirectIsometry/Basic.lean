...
def applyPolygon (iso : DirectIsometry) (poly : ConvexPolygon) : ConvexPolygon where
  ...
  vertices_extremeRationalPoints := by
    intro i j k hij hjk
    have h := poly.vertices_extremeRationalPoints i j k hij hjk
    simp only [RationalPoint.ccw, RationalPoint.isStrictlyLeftOf] at h ⊢
    -- Rotations preserve cross products: det of rotation matrix = cos²+sin² = 1
    have hcross : RationalPoint.crossProduct
          (iso.apply (poly.vertices j) - iso.apply (poly.vertices i))
          (iso.apply (poly.vertices k) - iso.apply (poly.vertices i)) =
        RationalPoint.crossProduct
          (poly.vertices j - poly.vertices i)
          (poly.vertices k - poly.vertices i) := by
      simp only [RationalPoint.crossProduct, apply, Pi.sub_apply, Pi.add_apply,
        Fin.isValue, Matrix.cons_val_zero, Matrix.cons_val_one]
      linear_combination
        ((poly.vertices j 0 - poly.vertices i 0) * (poly.vertices k 1 - poly.vertices i 1) -
         (poly.vertices j 1 - poly.vertices i 1) * (poly.vertices k 0 - poly.vertices i 0)) *
        iso.isValid
    rw [hcross]
    exact h
