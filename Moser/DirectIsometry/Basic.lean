...
This file defines planar isometries (rotations + translations) over an ordered field `K`.
-/
...
structure DirectIsometry (K : Type*) [Field K] [LinearOrder K] [IsStrictOrderedRing K] where
...
def id : DirectIsometry K := ⟨1, 0, 0, by grind⟩

/-- Apply the isometry to a point -/
def apply (iso : DirectIsometry K) (p : Point K) : Point K :=
  let rotated := ![iso.cos * p 0 - iso.sin * p 1, iso.sin * p 0 + iso.cos * p 1]
  rotated + iso.translation

/-- Applying a `DirectIsometry` is an injective function on `Point K`. -/
theorem apply_injective (iso : DirectIsometry K) : Function.Injective (iso.apply) := by
  intro x y h
...
/-- Applying a `DirectIsometry` is a surjective function on `Point K`. -/
theorem apply_surjective (iso : DirectIsometry K) : Function.Surjective (iso.apply) := by
  intro q
...
instance : FunLike (DirectIsometry K) (Point K) (Point K) where
  coe := apply
  coe_injective' := by
    intro i₁ i₂ h
    -- Apply to 0 to extract translation equality component-wise
    have h00 : i₁.apply (0 : Point K) 0 = i₂.apply (0 : Point K) 0 :=
      congr_fun (congr_fun h 0) 0
    have h01 : i₁.apply (0 : Point K) 1 = i₂.apply (0 : Point K) 1 :=
    ...
    have hcos_eq : i₁.apply (![1, 0] : Point K) 0 = i₂.apply (![1, 0] : Point K) 0 :=
      congr_fun (congr_fun h ![1, 0]) 0
    have hsin_eq : i₁.apply (![1, 0] : Point K) 1 = i₂.apply (![1, 0] : Point K) 1 :=
...
def applyPolygon (iso : DirectIsometry K) (poly : ConvexPolygon K) : ConvexPolygon K where
  ...
  vertices_extremePoints := by
    intro i j hji hji1
    have h := poly.vertices_extremePoints i j hji hji1
    ...
    -- hence preserve `ccw`.
    simp only [Point.ccw, Point.isStrictlyLeftOf, decide_eq_true_eq] at h ⊢
    have heq : Point.crossProduct
      ...
      simp only [Point.crossProduct, apply, Pi.sub_apply, Pi.add_apply,
...
def compose (iso1 iso2 : DirectIsometry K) : DirectIsometry K :=
