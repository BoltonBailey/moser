...
curves `[0,1] → ℝ²`, their `ε`-thickenings, the Moser cover number, and the
lower/upper bounds together with the planar Steiner formula bounding the gap
between them.

The finite grid `ε`-net (`thm:finiteEpsilonNet`) is built in
`Moser.Real.GridEpsilonNet`, which imports this file.
